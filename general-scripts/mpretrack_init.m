...
% frame - ID# for the image in the series %%MAKE IT A 4-DIGIT STRING WITH LEADING
%           0'S. e.g. '0001'
...
% 9/7/2023 JPC - changed frame input to strong and updated string
% conversions for reading input files
...
if ~exist(framesDir,'dir'), framesDir = fullfile(basepath,'tiffs'); end
files = dir(fullfile(framesDir,'*.tif*'));
if isempty(files), error('No TIFFs in %s', framesDir); end
files = natsortfiles(fullfile({files.folder},{files.name}));

frame = max(1, min(frame, numel(files)));
img = imread(files{frame});
%     if inv == 1
%         img2=img;
%         img = 255-img;
%     end
...
%     MT(d+1:a+d, 7)=time(frame);
    ...
    title('Features Found by Pretracking Algorithm');
    hold on
...
%     end
    ...
    format short g
    disp(M) % don't really want to see the whole matrix every time
...
[~, names, exts] = cellfun(@fileparts, paths, 'uni', 0); %#ok<ASGLU>
tokens = regexp(names, '(\d+)|(\D+)', 'match');
keys = cellfun(@(t) cellfun(@keyPart,t,'uni',0), tokens, 'uni', 0);
[~, idx] = sortrows(keys);
out = paths(idx);
    function k = keyPart(tok)
        d = sscanf(tok, '%d'); if ~isempty(d), k = {1, d}; else, k = {2, tok}; end
    end
end
end